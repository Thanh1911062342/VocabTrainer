...
export default function Trainer({ config, onReset, progressKey, configKey }) {
  ...
  const [showStudied, setShowStudied] = useState(false)   // <-- ensure state exists
  const [playingIdx, setPlayingIdx] = useState(null)      // audio playing index
  ...
  useEffect(() => {
    ...
    const currentPool = (progress.selectedIdxs && progress.selectedIdxs.length > 0)
      ...
      : words.slice(0, Math.min(progress.poolSize || 0, words.length))
    ...
    if (!entry) {
      try {
        ...
        audio.addEventListener('ended', () => {
          setPlayingIdx(prev => (prev === i ? null : prev))
        })
          ...
          <div className="modal-panel">
            ...
            <div className="modal-actions">
              <button
                ...
                title={lastResult.pass ? "Continue" : "Retry"}
                aria-label={lastResult.pass ? "Continue" : "Retry"}
              >
                          ...
                          <div className="studied-row-top flex justify-between text-sm">
                            <span className="flex items-center gap-2">
                              <button
                                type="button"
                                className="icon-btn"
                                onClick={(e) => { e.stopPropagation(); toggleAudioForIndex(i); }}
                                aria-label={playingIdx === i ? "Stop audio" : "Play audio"}
                                title={playingIdx === i ? "Stop" : "Play"}
                              >
                                {playingIdx === i ? <Square /> : <Volume2 />}
                              </button>
